...
export function RoomProvider({ children }: { children: ReactNode }) {
  ...
  const createRoom = useCallback(async (name: string): Promise<string> => {
    ...
    try {
      ...
      await set(roomRef(code), {
        ...
        createdAt: Date.now(),
        status: 'lobby',
        settings: { rounds: 5, includeCards: true, allowStealing: false, selectedPacks: [], turnTimeLimit: 0 },
        players: {
  ...
  const updateSettings = useCallback(async (partial: Partial<RoomSettings>) => {
    ...
    const current = settings || { rounds: 5, includeCards: true, allowStealing: false, selectedPacks: [] as string[], turnTimeLimit: 0 };
